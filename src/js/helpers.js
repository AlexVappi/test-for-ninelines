...
vars.ratio = window.innerWidth / window.innerHeight;
vars.share = {};

vars.share.setTitle = (text) => {
	if (!text || typeof text !== 'string') {
		return;
	}
	text = vars.clearText(text);
	$('.js-share-title').html(text);
	$('title').html(text);
	$('meta[property="og:title"]').attr('content', text);
	$('meta[name="twitter:title"]').attr('content', text);
};

vars.share.setDescription = (text) => {
	if (!text || typeof text !== 'string') {
		return;
	}
	text = vars.clearText(text);
	$('.js-share-desc').html(text);
	$('meta[name="description"]').attr('content', text);
	$('meta[property="og:description"]').attr('content', text);
	$('meta[name="twitter:description"]').attr('content', text);
};
